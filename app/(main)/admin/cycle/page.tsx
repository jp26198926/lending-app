      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            searchable: false,
            render: (cycle: Cycle) => {
              // Helper function: Check if there's a newer cycle for the same loan
              const hasNewerCycleForSameLoan = () => {
                return cycles.some(
                  (c) =>
                    c.loanId._id === cycle.loanId._id &&
                    c.cycleCount > cycle.cycleCount,
                );
              };
              ...
              return (
                <div className="flex items-center justify-center gap-1 sm:gap-2">
                  ...
                  {/* Hide Activate button for:
                      ...
                      - Cancelled cycles if there's a newer cycle for the same loan
                  */}
                  {cycle.status !== "Completed" &&
                    canEdit &&
                    cycle.status !== "Expired" &&
                    cycle.status === "Cancelled" &&
                    !hasNewerCycleForSameLoan() && (
                      <button
                        onClick={() =>
                          handleActivateClick(
                            cycle._id,
