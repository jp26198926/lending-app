...
export default function RoleManagement() {
  ...
  const [showAddRolePermissionForm, setShowAddRolePermissionForm] =
    useState(false);
  const [rolePermissionFormData, setRolePermissionFormData] = useState({
    pageId: "",
    permissionId: "",
  });
  const [showDeleteRolePermissionModal, setShowDeleteRolePermissionModal] =
    useState(false);
  ...
  const handleRolePermissionsClick = async (role: Role) => {
    setSelectedRole(role);
    setShowRolePermissionsModal(true);
    setShowAddRolePermissionForm(false);
    setRolePermissionFormData({ pageId: "", permissionId: "" });
    await Promise.all([
  ...
  const fetchRolePermissions = async (roleId: string) => {
    try {
      ...
      const res = await fetch(`/api/admin/rolepermission?roleId=${roleId}`);
  ...
  const fetchPages = async () => {
    try {
      ...
      setPages(data.filter((p: Page) => p.status === "ACTIVE"));
    } catch (err) {
  ...
  const fetchPermissions = async () => {
    try {
      ...
      setPermissions(data.filter((p: Permission) => p.status === "ACTIVE"));
    } catch (err) {
  ...
  const handleAddRolePermission = async (e: React.FormEvent) => {
    ...
    const exists = rolePermissions.find(
      (rp) =>
        rp.pageId._id === rolePermissionFormData.pageId &&
        rp.permissionId._id === rolePermissionFormData.permissionId &&
        rp.status !== "DELETED"
    );

    if (exists) {
      setError("This role-permission combination already exists");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(errorData.error || "Failed to add role permission");
      }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to add role permission"
      );
    } finally {
      setRolePermissionsLoading(false);
    }
  ...
  const handleDeleteRolePermissionClick = (id: string) => {
    setDeleteRolePermissionTarget(id);
    setShowDeleteRolePermissionModal(true);
  };

  const handleDeleteRolePermissionConfirm = async (reason?: string) => {
    if (!deleteRolePermissionTarget || !reason || !selectedRole) return;

    try {
      setRolePermissionsLoading(true);
      const res = await fetch(
        `/api/admin/rolepermission/${deleteRolePermissionTarget}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(
          errorData.error || "Failed to delete role permission"
        );
      }

      await fetchRolePermissions(selectedRole._id);
      setShowDeleteRolePermissionModal(false);
      setDeleteRolePermissionTarget(null);
      setError(null);
    } catch (err) {
      setError(
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <Modal
        ...
        title={`Manage Permissions - ${selectedRole?.role || ""}`}
        size="xl"
      >
        <div className="space-y-6">
          ...
          {/* Add Permission Form - Toggleable */}
          <div className="border-b pb-4">
            {!showAddRolePermissionForm ? (
              <button
                onClick={() => {
                  setShowAddRolePermissionForm(true);
                  setError(null);
                }}
                className="w-full px-4 py-3 bg-zentyal-primary text-white rounded-lg hover:bg-zentyal-dark 
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <form onSubmit={handleAddRolePermission} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Page <span className="text-red-500">*</span>
                    </label>
                    <select
                      value={rolePermissionFormData.pageId}
                      onChange={(e) =>
                      ...
                      required
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 
                               focus:ring-zentyal-primary focus:border-transparent"
                    >
                  ...
                  <div>
                    ...
                    <select
                      value={rolePermissionFormData.permissionId}
                      onChange={(e) =>
                        setRolePermissionFormData({
                          ...rolePermissionFormData,
                  ...
                  <div className="flex gap-3 justify-end">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-zentyal-primary text-white rounded-lg hover:bg-zentyal-dark 
                               transition-all shadow-md hover:shadow-lg disabled:opacity-50"
                    >
                      {rolePermissionsLoading ? "Adding..." : "Add Permission"}
                    </button>
                  </div>
                </form>
              </div>
                      ...
                      {rolePermissions.map((rp) => (
                        <tr key={rp._id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm">
                            <span className="px-2 py-1 bg-zentyal-primary/10 text-zentyal-primary rounded-full text-xs font-medium">
      ...
      {/* Delete Role Permission Confirmation Modal */}
      <ConfirmModal
        isOpen={showDeleteRolePermissionModal}
        onClose={() => {
          setShowDeleteRolePermissionModal(false);
          setDeleteRolePermissionTarget(null);
        }}
        onConfirm={handleDeleteRolePermissionConfirm}
        title="Delete Role Permission"
        message="Are you sure you want to delete this role permission? This will affect user access."
        confirmText="Delete"
        cancelText="Cancel"
        requireReason={true}
